...
public class ADIS16448_IMU extends GyroBase implements Gyro, PIDSource, Sendable {
	private static final double kCalibrationSampleTime = 4.0; // Calibration time in seconds.
	 ...
	 *            Which axis is Yaw
	 * @param algorithm
	 *            Use {@link #calculateComplementary} or
	...
	private void acquire() {
		...
		int array_offset = 0;
		int imu_crc = 0;
		double dt = 0.009765625; // This number must be adjusted if decimation
									// setting is changed. Default is 1/102.4
									// SPS
		int data_subset[] = new int[28];

		while (!m_freed.get()) {
			// Waiting for the buffer to fill...
			Timer.delay(.020); // A delay less than 10ms could potentially
								// overflow the local buffer

			data_count = m_spi.readAutoReceivedData(buffer, 0, 0); // Read
																	// number of
																	// bytes
																	// currently
																	// stored in
			...
			m_spi.readAutoReceivedData(buffer, data_count, 0); // Read data from
																// DMA buffer
			for (int i = 0; i < data_count; i += 28) { // Process each set of 28
														// bytes

				for (int j = 0; j < 28; j++) { // Split each set of 28 bytes
												// into a sub-array for
												// processing
				 ...
				 * "," + ToUShort(data_subset[2], data_subset[3]) + "," +
				 * ToUShort(data_subset[4], data_subset[5]) + "," +
				 * ToUShort(data_subset[6], data_subset[7]) + "," +
				 * ToUShort(data_subset[8], data_subset[9]) + "," +
				 * ToUShort(data_subset[10], data_subset[11]) + "," +
				 * ToUShort(data_subset[12], data_subset[13]) + "," +
				 * ToUShort(data_subset[14], data_subset[15]) + "," +
				 * ToUShort(data_subset[16], data_subset[17]) + "," +
				 * ToUShort(data_subset[18], data_subset[19]) + "," +
				 * ToUShort(data_subset[20], data_subset[21]) + "," +
				 * ToUShort(data_subset[22], data_subset[23]) + "," +
				 * ToUShort(data_subset[24], data_subset[25]) + "," +
				...
				int read_byte = 0;
				for (int k = 4; k < 26; k += 2) { // Cycle through XYZ GYRO, XYZ
													// ACCEL, XYZ MAG, BARO,
													// TEMP (Ignore Status &
													// CRC)
				...
				// Compare calculated vs read CRC. Don't update outputs if
				// CRC-16 is bad
				if (calc_crc == imu_crc) {
					...
					/*
					 * System.out.println(gyro_x + "," + gyro_y + "," + gyro_z +
					 * "," + accel_x + "," + accel_y + "," + accel_z + "," +
					 * mag_x + "," + mag_y + "," + mag_z + "," + baro + "," +
					 * temp + "," + "," + ToUShort(data_subset[26],
					 * data_subset[27]));
					 */
					// System.out.println("---------------------"); // Frame
					// divider (or else data looks like a mess)
